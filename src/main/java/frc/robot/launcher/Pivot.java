...
public class Pivot {
    ...
    public void run() {
        ...
        workShuffleBoard();
        if (Math.abs(Constants.operatorController.getY(Hand.kLeft)) > Constants.kTriggerThreshold) {
            teleopRun();
        } else if (Constants.operatorController.getStickButton(Hand.kRight)) {
            align();
        } else if (Constants.operatorController.getXButton()) {
            callibrate();
        }  else if (Constants.operatorController.getStickButton(Hand.kLeft)) {
            reset();
            
        }   else {
            // DPad controls
            switch (Constants.operatorController.getPOV()) {
                case 0:
                    // Up
                    setAgainst();
                    break;
    ...
    public void teleopRun() {
        ...
        if (!lowerLimit.get() && Constants.operatorController.getY(Hand.kLeft) > 0) {
            callibrate();
            return;
        }

        if (getRevolution() < Constants.PIVOT_ZERO_THRESHOLD && Constants.operatorController.getY(Hand.kLeft) > 0) {
            sparkA.set(Constants.operatorController.getY(Hand.kLeft) * Constants.PIVOT_ZERO_SPEED);
            return;
        }

        if (getRevolution() > Constants.PIVOT_MAX_REVOLUTION && Constants.operatorController.getY(Hand.kLeft) < 0) {
            return;

        }
        sparkA.set(Constants.operatorController.getY(Hand.kLeft) * Constants.PIVOT_TELEOP_SPEED);
    }
